<template>
  <div>
    <div class="box">
      ...
      <div class="control is-horizontal" v-if="paidToDisabled">
        <div class="control-label">
  ...
  export default {
    ...
    methods: {
      ...
      submit () {
        ...
        axios.post('/petty-cash/add', this.cmd)
          .then((resp) => {
      ...
      fetchData () {
        ...
        ]).then(axios.spread((fundingList, roleDetail, employeeList) => {
          this.roleName = roleDetail.data.kode
          console.log('testes', this.roleName)
          if (this.roleName === 'ACCOUNTING_STAFF' || this.roleName === 'DIRECTOR' || this.roleName === 'DEPUTY_DIRECTOR') {
            this.paidToDisabled = true
          } else {
            this.cmd.paidTo = auth.getUserInfo().employeeId
          }
