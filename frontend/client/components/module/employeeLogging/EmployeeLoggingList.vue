...
import EmployeeLoggingOperation from './EmployeeLoggingOperation.vue'
...
Vue.component('EmployeeLoggingOperation', EmployeeLoggingOperation)
...
let tableColumns = [
  ...
  },
  {
    title: 'Action',
    titleClass: 'center aligned',
    name: '__component:employee-logging-operation',
    dataClass: 'center aligned',
    width: '150px'
  }
]
...
export default {
  ...
  methods: {
    fetchData () {
      ...
      this.parentList = []
      axios.all([
        // axios.get('/employee/get/projectId/' + auth.getUserInfo().employeeId + '/' + this.form.months + '/' + this.form.years),
        axios.get('/employee/aj'),
        axios.get('/sub-project-by-project/3')
      ]).then(axios.spread((employeeList, parentList) => {
        ...
        for (let parent of parentList.data) {
          this.parentList.push({value: parent.id, text: parent.code, spv: parent.spvName})
        }
      }))
    },
    reset () {
      ...
      axios.post('/log-act-detail/checkProjectEmployee', this.form)
        .then((resp) => {
          ...
          if (this.form.projectAssignId !== '') {
            axios.post('/log-act-detail', this.form)
              .then((resp) => {
                bus.$emit('HIDE_LOADING')
                console.log('SUCCESS')
                console.log(resp)
            ...
            bus.$emit('HIDE_LOADING')
            bus.$emit('NOTIFY', 'danger', 'Tidak memiliki project assignment di tanggal ' + this.form.purchaseDateString)
          }
        })
      .catch((error) => {
    ...
    onEmployeeSelect (item) {
      ...
      this.search()
      // axios.all([
      //   axios.get('/sub-pro-assignment/' + item.value)
      // ]).then(axios.spread((parentList) => {
      //   for (let parent of parentList.data) {
      //     this.parentList.push({value: parent.id, text: parent.subProName, spv: parent.spvName})
      //   }
      // }))
    },
    onParentSelect (item) {
