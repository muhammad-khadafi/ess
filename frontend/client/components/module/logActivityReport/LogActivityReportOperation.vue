...
import LogActivityReportForm from './LogActivityReportForm.vue'
import LogActivityReportDelete from './LogActivityReportDelete.vue'
...
export default {
  components: {
    Tooltip,
    Modal,
    LogActivityReportForm,
    LogActivityReportDelete
  },
  data () {
    return {
      visible: false,
  ...
  methods: {
    ...
    onClick (action, data) {
      ...
      if (action === 'edit-item') {
        ...
        let root = window.location.protocol + '//' + window.location.host
        console.log('Ready to generate')
        window.open(root + '/api/download/log-activity-report/get-report/' + data.id + '/' + data.name)
        ...
        this.contentComponent = 'log-activity-report-delete'
        this.onOpen()
      }
    }
  }
