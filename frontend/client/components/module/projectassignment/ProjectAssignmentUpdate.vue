    ...
    methods: {
      submit () {
        ...
        axios.post('/project-assignment/checkduplicate', this.cmd)
          .then((resp) => {
            if (resp.data === false) {
              axios.put('/project-assignment/' + this.cmd.id, this.cmd)
                .then((resp) => {
                  bus.$emit('HIDE_LOADING')
                  console.log('SUCCESS')
                  console.log(resp)
      ...
      validateAll () {
        ...
        if (this.cmd.load !== null && (this.cmd.load < 1 || this.cmd.load > 100)) {
      ...
      fetchData () {
        ...
        axios.all([
          ...
          axios.get('/project-assignment/view/' + this.$route.params.id)
            .then((resp) => {
              this.cmd = resp.data
