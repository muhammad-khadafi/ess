...
import InvoiceReportDelete from './InvoiceReportDelete.vue'
  ...
  methods: {
    ...
    onClick (action, data) {
      ...
      if (action === 'edit-item') {
        ...
        //   })
        let root = window.location.protocol + '//' + window.location.host
        console.log('Ready to generate Invoice Reports')
        window.open(root + '/api/download/invoice-report/get-report/' + data.id)
        ...
        this.contentComponent = 'invoice-report-delete'
        this.onOpen()
      }
    }
  }
