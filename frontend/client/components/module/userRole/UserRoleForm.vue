    ...
    methods: {
      fetchData () {
        axios.all([
          ...
          axios.get('/userWeb/getAll')
        ]).then(axios.spread((roleList, userList) => {
          for (let roles of roleList.data) {
            this.roleList.push({value: roles.id, text: roles.label})
          }
          for (let users of userList.data) {
            this.userList.push({value: users.id, text: users.username})
          }
        }))
      },
      onRoleSelect (item) {
      ...
      submit () {
        ...
        axios.post('/user-role/add', this.userWebRole)
          .then((resp) => {
            bus.$emit('HIDE_LOADING')
            console.log('SUCCESS')
            console.log(resp)
