...
import RecapLogActivityReportDelete from './RecapLogActivityReportDelete.vue'
  ...
  methods: {
    ...
    onClick (action, data) {
      ...
      if (action === 'edit-item') {
        ...
        let root = window.location.protocol + '//' + window.location.host
        console.log('Ready to generate')
        window.open(root + '/api/download/recap-log-activity-report/get-report/' + data.id + '/' + data.name)
        ...
        this.contentComponent = 'recap-log-activity-report-delete'
        this.onOpen()
      }
    }
  }
