        ...
        <div class="control is-grouped">
          <div class="control is-expanded">
            ...
            <input class="input" :class="{'is-danger' : errors.purchasePrice}"
                    type="text" v-on:keyup="onBlurNumber" v-model="pricewithSeparator" placeholder="Purchase Price"/>
            </div>
          </div>
...
export default {
  ...
  created () {
    ...
    .then((resp) => {
      this.cmd = resp.data
      this.pricewithSeparator = this.cmd.purchasePrice.toString().replace(/\B(?=(\d{3})+(?!\d))/g, '.')
      this.inventoryTypeItem.text = this.cmd.typeString
    })
  ...
  methods: {
    ...
    submit () {
      ...
      axios.put('/inventory/' + this.cmd.id, this.cmd)
      .then((resp) => {
        bus.$emit('HIDE_LOADING')
    ...
    thousandSeparator (amount) {
      this.cmd.purchasePrice = this.pricewithSeparator.replace(/\D/g, '')
